...
class RNBase64GrayscaleModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    @ReactMethod
    private fun base64GrayscaletoBinaryImageScaled(base64: String, width:Int, promise:Promise) {
        try{
            val code = Base64.decode(base64, Base64.DEFAULT)
            val test = BitmapFactory.decodeByteArray(code, 0, code.size)
            println("This is test.width ${test.width}")
            println("This is test.height ${test.height}")
            val calculatedHeight=(test.height * width)/test.width
            println("This is calculatedHeight ${calculatedHeight}")
            val scaled= Bitmap.createScaledBitmap(test,width,calculatedHeight,false)
            var binaryImageString= StringBuilder() //uses string builder instead of concatenating because
            // if we were to concat using += its slow and creates a new instance for every concat Stringbuilder doesn't
...
//                    var finalInt= Color.argb(A,R,G,B)
    ...
    @ReactMethod
    private fun  base64toScaledBitmap(base64: String, promise:Promise){
        val encodedBase64 = Base64.decode(base64, Base64.DEFAULT)
        val bitmap = BitmapFactory.decodeByteArray(encodedBase64, 0, encodedBase64.size)
        val n = Math.ceil(bitmap.width.toDouble()/ 8).toInt();
        val scaledBitmap= Bitmap.createScaledBitmap(bitmap,500,281, false)
        val byteArrayOutputStream = ByteArrayOutputStream()
